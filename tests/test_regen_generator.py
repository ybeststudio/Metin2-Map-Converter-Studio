...
def test_build_regen_text_prefers_matching_sample(tmp_path) -> None:
    ...
    text, source = build_regen_text(map_source, sample_root=tmp_path / "samples")

    assert text == "r\t100\t200\t10\t10\t0\t0\t5s\t100\t1\t101\n"
    assert source == "sample:metin2_map_test"


def test_build_regen_text_uses_parent_map_sample(tmp_path) -> None:
    source_root = tmp_path / "client"
    ...
    sample_dir.mkdir(parents=True)
    (sample_dir / "regen.txt").write_text(
        "m 150 250 20 20 0 0 30s 100 1 201\n",
        encoding="utf-8",
    )
    ...
    assert text == "m\t150\t250\t20\t20\t0\t0\t30s\t100\t1\t201\n"
    assert source == "sample:metin2_map_parent"


def test_build_regen_text_returns_empty_fallback_when_missing(tmp_path) -> None:
    source_root = tmp_path / "client"
    map_dir = source_root / "metin2_map_test"
    map_dir.mkdir(parents=True)
    (map_dir / "setting.txt").write_text(VALID_SETTING, encoding="utf-8")
